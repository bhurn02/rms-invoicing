...
$dtPastDueAccount = date("m/d/Y",(strtotime(date("m/d/y", time()))+60*60*24*($OFFSET)));
...
if ($strMode != "") {
	...
	$dtPastDueAccount = $_POST["DPC_txtPastDueAccount"];
	...
	switch ($strMode) {
		case "SEARCH":		
			$sqlquery="exec sp_rpt_TenantForPaymentNoticeFinal_Search '" . $dtAsOf . "','" . $strSortBy . "','" . $strSearchValue . "'";	
			$process=odbc_exec($sqlconnect, $sqlquery);
			//echo $sqlquery;
			break;
			
		case "PRINT":		
			$sqlquery="exec sp_rpt_TenantForPaymentNoticeFinal_Save 'DELETE',0";	
			$process=odbc_exec($sqlconnect, $sqlquery);					
			$i = 0;
			while ($i <= $_POST["hidRowCtr"]) {
				if (isset($_POST["chkSelect" . strval($i)])) {					
					$strCode = $_POST["hidCode" . strval($i)];
					$sqlquery="exec sp_rpt_TenantForPaymentNoticeFinal_Save 'SAVE'," . $strCode . "";	
					//echo $sqlquery;
					$process=odbc_exec($sqlconnect, $sqlquery);				
				}
				$i++;
			...
			$my_report = $report_path . "t_tenant_demand_notice_final.rpt"; // 
			//echo $my_report;
			//die();
			//rpt source file 
			$pdf_file = "t_tenant_demand_notice_final" . str_replace("/","",date("m/d/y/H/i/s", time())) . ".pdf";
			...
			$creport->ParameterFields(3)->AddCurrentValue ($_POST["DPC_txtPastDueAccount"]);
			$creport->ParameterFields(4)->AddCurrentValue ($_POST["DPC_txtPaymentDue"]);
...
<html> 
<head> 
<title>NOTICE > FINAL DEMAND TO PAY - FIRST NOTICE</title> 
...
<form name="frmFinalNotice" id="frmFinalNotice" method="post">
	<div class="mainmenu">	
		<ul>
			  <li class="li_nc"><a name="MODULE NAME">NOTICE > DEMAND TO PAY - FINAL NOTICE
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			 </a></li>	
						...
						<tr>
							<td>
								<table>
									...
									<tr>
										<td class="fieldname_right">PAST DUE ACCOUNT DATE (mm/dd/yyyy):</td>
										<td width="20"></td>
										<td><input type=text name="DPC_txtPastDueAccount" id="DPC_txtPastDueAccount" class="values" size="20" maxlength="10" value="<?php echo $dtPastDueAccount;?>">										
										</td>
									</tr>									
									<tr>
...
function save_text()
   {
   var w = frmFinalNotice.cboRealProperty.selectedIndex;
   frmFinalNotice.hidRealPropertyName.value = frmFinalNotice.cboRealProperty.options[w].text;
   cmdSearch_OnClick();
   }
...
function cmdSearch_OnClick() {
	if (frmFinalNotice.DPC_txtAsOf.value == "") {
		alert("Aging As Of is required")		
		frmFinalNotice.DPC_txtAsOf.focus()
		return false
	}
	
	if (frmFinalNotice.DPC_txtAsOf.value != "") {
		if (isDate(frmFinalNotice.DPC_txtAsOf.value)==false) {
			frmFinalNotice.DPC_txtAsOf.focus()
			return false
		}
	}
	
	frmFinalNotice.hidMode.value = "SEARCH"
	frmFinalNotice.submit()
}

function cmdPrint_OnClick() {

	if (frmFinalNotice.DPC_txtNotice.value == "") {
		alert("Notice Date is required")		
		frmFinalNotice.DPC_txtNotice.focus()
		return false
	}
	
	if (frmFinalNotice.DPC_txtNotice.value != "") {
		if (isDate(frmFinalNotice.DPC_txtNotice.value)==false) {
			frmFinalNotice.DPC_txtNotice.focus()
			return false
		}
	}
	
	if (frmFinalNotice.DPC_txtPaymentDue.value == "") {
		alert("Payment Due Date is required")		
		frmFinalNotice.DPC_txtPaymentDue.focus()
		return false
	}

	if (frmFinalNotice.DPC_txtPaymentDue.value != "") {
		if (isDate(frmFinalNotice.DPC_txtPaymentDue.value)==false) {
			frmFinalNotice.DPC_txtPaymentDue.focus()
			return false
		}
	}
	...
	var j
	j=0
	totalctr = frmFinalNotice.hidRowCtr.value;
	for (i=1;i<=totalctr;i++) {
		obj = eval("frmFinalNotice.chkSelect" + i);
		if (obj.checked == true) {
			j++;
		}
	}
	...
	if (j > 0) {
		frmFinalNotice.hidMode.value = "PRINT"
		frmFinalNotice.submit()
	}
	else {
...
function chkSelectAll_OnClick() {
	var ctr
	ctr = frmFinalNotice.hidRowCtr.value;
	for (i=1;i<=ctr;i++) {
		obj = eval("frmFinalNotice.chkSelect" + i);
		if (frmFinalNotice.chkSelectAll.checked == true) {
			obj.checked = true;
		}
		else {
			obj.checked = false;
