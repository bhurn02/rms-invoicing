...
if ($strMode == "DELETE") {
	...
	while ($i <= $_POST["hidRowCtr"]) {
		if (isset($_POST["chkDelete" . strval($i)])) {
			$sqlquery="exec sp_s_User_Group 'DELETE'," . $_POST["hidGroupCode" . strval($i)] . ",'','','" . $sessUserID . "','" . $strIPAddr . "'";
			//echo $sqlquery;
...
<form id="frmUserGroupList" name"frmUserGroupList" method="post">
	<div class="mainmenu">	
		<ul>
			 ...
			 <li class="li_nc"><a href="#" onClick="javascript:change_loc('user_group.php?menu_id=<?php echo $menu_id;?>')">Back&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|</a></li>	  
		</ul>
	</div>
	<table>
		<tr>
				...
				<td>
					<table width="400" style="border:1px solid #556b2f;padding: 30px 10px 5px;">
						...
						<tr bgcolor="<?php echo "$rowColor" ?>">							
							<td width="12%" class="values" align="center">
								<input type="checkbox" name="chkDelete<?php echo "$ctr"?>" id="chkDelete<?php echo "$ctr"?>" value="<?php echo "$ctr"?>">								
							</td>
							<td width="26%" class="values">&nbsp;<a href="user_group.php?menu_id=<?php echo $menu_id;?>&group_code=<?php echo "$group_code";?>&mode=FIND"><?php echo "$group_code";?></a>&nbsp;
							</td>
