...
$sqlquery="exec sp_m_Tenant 'CLIENT_SEARCH','','" . $strKeyword . "','','','','','','','','','','','','','','','','','','','','','','','',''";
$process=odbc_exec($sqlconnect, $sqlquery);
				...
				<td>
					<table width="700" style="border:1px solid #556b2f;padding: 30px 10px 5px;">
						...
						while(odbc_fetch_row($process)){
							$tenant_code = replacesinglequote2(odbc_result($process,"tenant_code")); 
							$tenant_name = replacesinglequote2(odbc_result($process,"tenant_name")); 
							$real_property_code = replacesinglequote2(odbc_result($process,"real_property_code")); 
...
function tenant(pTenantCode,pTenantName) {
	window.opener.frmTenant.hidBillTo.value = pTenantCode;
	window.opener.frmTenant.hidBillToName.value = pTenantName;
	window.opener.frmTenant.txtBillTo.value = pTenantName;
	window.opener.frmTenant.hidBillToCodeTmp.value = pTenantCode;
	window.opener.frmTenant.hidBillToNameTmp.value = pTenantName;
	window.close();
}
