...
if ($strMode == "VOID") {
	...
	while ($i <= $_POST["hidRowCtr"]) {
		if (isset($_POST["chkSelect" . strval($i)])) {
			$sqlquery="exec sp_t_Security_Deposit 'VOID','" . $_POST["hidCode" . $i] . "','','',0,'','','','','','',0,0,'" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";
			//echo $sqlquery;
...
elseif ($strMode == "DELETE") {
	...
	while ($i <= $_POST["hidRowCtr"]) {
		if (isset($_POST["chkSelect" . strval($i)])) {
			$sqlquery="exec sp_t_Security_Deposit 'DELETE','" . $_POST["hidCode" . $i] . "','','',0,'','','','','','',0,0,'" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";
			//echo $sqlquery;
			$process=odbc_exec($sqlconnect, $sqlquery);
			if (odbc_fetch_row($process)) {
				if (odbc_result($process,"x") == 1) 
	...
	elseif ($j > 0 && $k == 0) {
		$strMsg = "No record deleted because it had been posted or with auto-generated no.!";
	}
	else
...
elseif ($strMode == "POST") {
	...
	while ($i <= $_POST["hidRowCtr"]) {
		if (isset($_POST["chkSelect" . strval($i)])) {
			$sqlquery="exec sp_t_Security_Deposit 'POST','" . $_POST["hidCode" . $i] . "','','',0,'','','','','','',0,0,'" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";
			//echo $sqlquery;
			$process=odbc_exec($sqlconnect, $sqlquery);
			if (odbc_fetch_row($process)) {
				if (odbc_result($process,"x") == 1) 
...
if ($src =="home") {
	$sqlquery="exec sp_t_Security_Deposit_List 'VIEW_STAT','" . $strSearchList . "','" . $strKeyword . "'," . $function_id . "";
}
else {
	$sqlquery="exec sp_t_Security_Deposit_List 'VIEW','" . $strSearchList . "','" . $strKeyword . "'," . $function_id . "";
}
$process=odbc_exec($sqlconnect, $sqlquery);
//echo $sqlquery;
...
<form id="frmTenantDepositList" name"frmTenantDepositList" method="post" action="tenant_deposit_list.php?menu_id=<?php echo $menu_id;?>">
	<div class="mainmenu">	
		<ul>
			  <li class="li_nc"><a name="MODULE NAME">TENANT SECURITY - FIND
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  
			 </a></li>	
			 ...
			 <li class="li_nc"><a href="#" onClick="javascript:change_loc('tenant_deposit.php?menu_id=<?php echo $menu_id;?>')">Back&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|</a></li>	  
		</ul>
	</div>
	<table>
		<tr>
				...
				<td>
					<table width="600" style="border:1px solid #556b2f;padding: 30px 10px 5px;">
						...
						<tr bgcolor="<?php echo "$rowColor"; ?>">							
							...
							<td width="20%" class="values">&nbsp;<a href="tenant_deposit.php?menu_id=<?php echo $menu_id;?>&or_no=<?php echo "$or_no";?>&mode=FIND"><?php echo "$or_no";?></a>&nbsp;
							</td>
...
function cmdVoid_OnClick() {
	if (confirm("Are you sure you want to void this record/s?")) {
		frmTenantDepositList.hidMode.value = "VOID";
		frmTenantDepositList.submit();
	}
}
...
function cmdDelete_OnClick() {
	if (confirm("Are you sure you want to delete this record/s?")) {
		frmTenantDepositList.hidMode.value = "DELETE";
		frmTenantDepositList.submit();
	}
}
