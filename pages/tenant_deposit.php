	...
	switch ($strMode) {
		case "SAVE":
			if ($strSaveMode != "EDIT") {
				...
				$sqlquery="exec sp_t_Security_Deposit 'FIND','" . $strORNo . "','" . $dtOR . "','" . $strClientCode . "'," . $dblAmount . ",'" . $strRemarks . "','" . $strStatus . "',0,'',0,'','','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
			...
			//die();
			if ($strMsg == "") {
				$sqlquery="exec sp_t_Security_Deposit 'SAVE','" . $strORNo . "','" . $dtOR . "','" . $strClientCode . "'," . $dblAmount . ",'" . $strRemarks . "','" . $strStatus . "',0,'',0,'','','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
				$process=odbc_exec($sqlconnect, $sqlquery);
				$strMsg = "Record saved!";					
				//echo $sqlquery;
				if ($strSaveMode != "EDIT") {
		...
		case "DELETE":
			$sqlquery="exec sp_t_Security_Deposit 'DELETE','" . $strORNo . "','" . $dtOR . "','" . $strClientCode . "',0,'" . $strDocumentNo . "','','','" . $strRemarks . "','" . $strStatus . "',0,'',0,'','','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
			$process=odbc_exec($sqlconnect, $sqlquery);
			if (odbc_fetch_row($process)) {
				if (odbc_result($process,"x") == 1) 
...
if ($strMode!= "") {
	if ($strMode=="SAVE") {
		//$sqlqueryPaymentMode="exec sp_t_Security_Deposit 'DELETE_PAYMENT_MODE','" . $strORNo . "','" . $dtOR . "','" . $strClientCode . "',0,'" . $strDocumentNo . "','" . $strPaymentModeType . "','" . $strBankName . "','" . $strRemarks . "','" . $strStatus . "','" . $strAccountNo . "',0,0,'" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
		...
		if ($_POST["hidIsCash"] == 1 || $_POST["hidIsCharge"] == 1 || $_POST["hidIsCheck"] == 1) {
			while ($i <= $_POST["hidRowCtr"]) {
				...
				if ($strPaymentModeType == "1") {
					...
					$strAccountNo = $_POST["txtEditAccountNo" . $i];
					$strBankName = $_POST["txtEditBankName" . $i];
				}
				$sqlqueryPaymentMode="exec sp_t_Security_Deposit 'SAVE_PAYMENT_MODE','" . $strORNo . "','" . $dtOR . "','" . $strClientCode . "'," . $dblAmount . ",'" . $strRemarks . "','" . $strStatus . "',0,'',0,'','','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
				$processPaymentMode=odbc_exec($sqlconnect, $sqlqueryPaymentMode);
				//echo $sqlqueryPaymentMode;
				$i++;
			}
		...
		if ($_POST["hidIsCharge"] == 0) {
			if ($_POST["txtEditAmountCharge"] > 0) {
					...
					$strAccountNo = $_POST["txtEditAccountNoCharge"];
					$strBankName = $_POST["txtEditBankNameCharge"];
					$sqlqueryPaymentMode="exec sp_t_Security_Deposit 'SAVE_PAYMENT_MODE','" . $strORNo . "','" . $dtOR . "','" . $strClientCode . "'," . $dblAmount . ",'" . $strRemarks . "','" . $strStatus . "',0,'',0,'','','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
					$processPaymentMode=odbc_exec($sqlconnect, $sqlqueryPaymentMode);
					//echo $sqlqueryPaymentMode;
			}
		...
		if ($_POST["hidIsCheck"] == 0) {
			if ($_POST["txtEditAmountCheck"] > 0) {
					...
					$strAccountNo = $_POST["txtEditAccountNoCheck"];
					$strBankName = $_POST["txtEditBankNameCheck"];
					$sqlqueryPaymentMode="exec sp_t_Security_Deposit 'SAVE_PAYMENT_MODE','" . $strORNo . "','" . $dtOR . "','" . $strClientCode . "'," . $dblAmount . ",'" . $strRemarks . "','" . $strStatus . "',0,'',0,'','','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
					$processPaymentMode=odbc_exec($sqlconnect, $sqlqueryPaymentMode);
					//echo $sqlqueryPaymentMode;
				}
		...
		if ($_POST["txtAddAmount"] > 0) {
				...
				$strAccountNo = $_POST["txtAddAccountNo"];
				$strBankName = $_POST["txtAddBankName"];
				$sqlqueryPaymentMode="exec sp_t_Security_Deposit 'SAVE_PAYMENT_MODE','" . $strORNo . "','" . $dtOR . "','" . $strClientCode . "'," . $dblAmount . ",'" . $strRemarks . "','" . $strStatus . "',0,'',0,'','','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
				$processPaymentMode=odbc_exec($sqlconnect, $sqlqueryPaymentMode);
				//echo $sqlqueryPaymentMode;
			}
	...
	if ($strMode == "RETRIEVE" || $strMode == "FIND") {
		$sqlquery="exec sp_t_Security_Deposit_Retrieve 'RETRIEVE','" . $strORNo . "'";	
		//echo $sqlquery;
		$process=odbc_exec($sqlconnect, $sqlquery);
		if (odbc_fetch_row($process)) {
			$strORNo = odbc_result($process,"or_no");
	...
	if ($strMode=="RETRIEVE" || $strMode=="FIND" || $strMode=="SAVE") {
		$sqlqueryPaymentMode="exec sp_t_Security_Deposit 'RETRIEVE_PAYMENT_MODE','" . $strORNo . "','" . $dtOR . "','" . $strClientCode . "'," . $dblAmount . ",'" . $strRemarks . "','" . $strStatus . "',0,'',0,'','','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
		$processPaymentMode=odbc_exec($sqlconnect, $sqlqueryPaymentMode);
		//echo $sqlqueryPaymentMode;
	}
}
...
<html> 
<head> 
<title>TENANT SECURITY DEPOSIT</title> 
...
<form name="frmTenantDeposit" id="frmTenantDeposit" method="post" action="tenant_deposit.php?menu_id=<?php echo $menu_id;?>">
	<div class="mainmenu">	
		<ul>
			 <li class="li_nc"><a name="MODULE NAME">TENANT SECURITY DEPOSIT
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			 </a></li>
			  ...
			  <?php } ?>				   
			  <li class="li_nc"><a href="#" onClick="javascript:change_loc('tenant_deposit_list.php?menu_id=<?php echo $menu_id;?>')">Find&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|</a></li>
			  ...
			  <?php } else { ?>	
			  	<li class="li_nc"><a href="#" target="_self" style="color: #FFFF33" onClick="javascript:change_loc('tenant_deposit_detail.php?menu_id=<?php echo $menu_id;?>&mode=FIND&menu=DETAIL&or_no=<?php echo $strORNo; ?>')">Detail&nbsp;&nbsp;&nbsp;&nbsp;<font color="white">|</font></a></li>
			  <?php } ?>	
		</ul>
	</div>
	<table>
							...
							<td>
								<table>
									...
									<tr>
										...
										<td width="20"></td>
										<td class="values">$<?php echo $dblTotalPaymentDetailAmount;?>										
										</td>
									</tr>																							
									<tr>
...
function cmdSave_OnClick() {
	if (trim(frmTenantDeposit.DPC_txtORDate.value) == "") {
		alert("OR Date is required")
		frmTenantDeposit.DPC_txtORDate.focus()
		return false
	}
	if (frmTenantDeposit.DPC_txtORDate.value != "") {
		if (isDate(frmTenantDeposit.DPC_txtORDate.value)==false){
			frmTenantDeposit.DPC_txtORDate.focus()
			return false
		}
	}
	if (trim(frmTenantDeposit.txtClientName.value) == "") {
		alert("Client is required")
		frmTenantDeposit.cmdClientSearch.focus()
		return false
	}
	...
	j=0
	dblTmpTotal = 0
	totalctr = frmTenantDeposit.hidRowCtr.value;
	for (i=1;i<=totalctr;i++) {
		obj = eval("frmTenantDeposit.txtEditAmount" + i);
		if (isNaN(obj.value)) {
	...
	if (frmTenantDeposit.hidIsCash.value == 0) {
		obj = eval("frmTenantDeposit.txtEditAmountCash");
		if (isNaN(obj.value)) {
			alert("Invalid numeric value")
			obj.focus()
			return false
	...
	if (frmTenantDeposit.hidIsCharge.value == 0) {		
		obj = eval("frmTenantDeposit.txtEditAmountCharge");	
		if (isNaN(obj.value)) {
			alert("Invalid numeric value")
			obj.focus()
			return false
	...
	if (frmTenantDeposit.hidIsCheck.value == 0) {		
		obj = eval("frmTenantDeposit.txtEditAmountCheck");	
		if (isNaN(obj.value)) {
			alert("Invalid numeric value")
			obj.focus()
			return false
	...
	obj = eval("frmTenantDeposit.txtAddAmount");	
	if (isNaN(obj.value)) {
		alert("Invalid numeric value")
		obj.focus()
		return false
	}	
	if (Number(obj.value) > 0 && frmTenantDeposit.cboAddPaymentMode.value=="") {
		alert("Mode of Payment is required")
		frmTenantDeposit.cboAddPaymentMode.focus()
		return false
	}
	...
	if (dblTmpTotal != frmTenantDeposit.hidTotalPaymentDetailAmount.value) {
		alert("Amounts' total should be equal to $" + frmTenantDeposit.hidTotalPaymentDetailAmount.value)
		return false;
	}	
	
	if (frmTenantDeposit.chkVoid.checked == true) {
		if (confirm("Are you sure you want to void this OR?")) {
			frmTenantDeposit.hidMode.value = "SAVE";
			frmTenantDeposit.submit();		
		}
	}
	else {
		frmTenantDeposit.hidMode.value = "SAVE";
...
function cmdRetrieve_OnClick() {
	frmTenantDeposit.hidORNo.value = frmTenantDeposit.txtORNo.value;
	frmTenantDeposit.hidMode.value = "RETRIEVE";
	frmTenantDeposit.submit();
}

function cmdPrint_OnClick() {
	frmTenantDeposit.hidMode.value = "PRINT";
	frmTenantDeposit.submit();
}

function cmdDelete_OnClick() {
	if (confirm("Are you sure you want to delete this record?")) {
		frmTenantDeposit.hidMode.value = "DELETE";
		frmTenantDeposit.submit();
	}
}

function cmdCancel_OnClick() {
	parent.frames[2].location = "tenant_deposit.php?menu_id=" + frmTenantDeposit.hidMenuID.value;
}
...
function cmdClientSearch_onClick() {
	window.open ("tenant_deposit_search_client.php?menu_id=" + frmTenantDeposit.hidMenuID.value,"displayWindow","type=fullwindow,titlebar=no,scrollbars=yes");
	return false;
}
