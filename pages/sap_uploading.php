...
if ($strMode != "") {	
	...
	switch ($strMode) {
		case "BUILD":						
			$sqlquery="exec sp_t_SAP_Uploading_Build_View 'BUILD','','" . $dtBillingFrom . "','" . $dtBillingTo . "','" . $strRealPropertyCode . "','" . $strSortBy . "','','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
		...
		case "POST":						
			...
			while ($i <= $_POST["hidRowCtr"]) {
				if (isset($_POST["chkSelect" . strval($i)])) {					
					...
					$sqlquery="exec sp_t_SAP_Uploading 'POST','" . $strInvoiceNo . "','" . $dtBillingFrom . "','" . $dtBillingTo . "','" . $strRealPropertyCode . "','','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
					//echo $sqlquery;
					$process=odbc_exec($sqlconnect, $sqlquery);					
					$j++;
				}
			...
			if ($j > 0)
				$strMsg = "Invoice/s posted!";
			else
				$strMsg = "No invoice posted!";
		...
		case "GENERATE":
			...
			$sqlquery="exec sp_t_SAP_Uploading 'DELETE','','" . $dtBillingFrom . "','" . $dtBillingTo . "','" . $strRealPropertyCode . "','" . $date_uploaded . "','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
			$process=odbc_exec($sqlconnect, $sqlquery);
			$strInvoiceNo = "";
			while ($i <= $_POST["hidRowCtr"]) {
				if (isset($_POST["chkSelect" . strval($i)])) {					
					if ($strInvoiceNo == "" || $strInvoiceNo != $_POST["hidInvoiceNo" . strval($i)]) {
						$strInvoiceNo = $_POST["hidInvoiceNo" . strval($i)];
						$sqlquery="exec sp_t_SAP_Uploading 'GENERATE','" . $strInvoiceNo . "','" . $dtBillingFrom . "','" . $dtBillingTo . "','" . $strRealPropertyCode . "','" . $date_uploaded . "','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
					//echo $sqlquery;
						$process=odbc_exec($sqlconnect, $sqlquery);
					}
					$j++;
			...
			if ($j == 0) {
				$strMsg = "No invoice selected!";
			}
			else {
				//create file
				$strSAPTextFileName = "SAP" .  str_replace("/","",date("m/d/y/H/i/s", time())) . ".prn" ;
				...
				$sqlquery="exec sp_t_SAP_Uploading 'CREATE_FILE','','" . $dtBillingFrom . "','" . $dtBillingTo . "','" . $strRealPropertyCode . "','" . $date_uploaded . "','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
				$process=odbc_exec($sqlconnect, $sqlquery);
				...
				$sqlquery="exec sp_t_SAP_Uploading 'DELETE','','" . $dtBillingFrom . "','" . $dtBillingTo . "','" . $strRealPropertyCode . "','" . $_POST["hidDateUploaded"] . "','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
				$process=odbc_exec($sqlconnect, $sqlquery);
...
<form name="frmSAPUploading" id="frmSAPUploading" method="post">
	<div class="mainmenu">	
		<ul>
			  <li class="li_nc"><a name="MODULE NAME">SAP UPLOADING
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				...
				<td>
					...
					<table width="900" style="border:1px solid #556b2f;padding: 30px 10px 5px; width:auto;">
						<tr height="30">
							...
							<?php
								$ctrCharge = 0;
								$sqlqueryCharges="select * from m_charges where charge_code in (select charge_code from z_tmp_sap_uploading_view where isnull(total_amount,0) > 0) order by charge_desc";	
						...
						<?php	
						$ctr = 0;		
						$sqlquery="exec sp_t_SAP_Uploading_Build 'BUILD','','" . $dtBillingFrom . "','" . $dtBillingTo . "','" . $strRealPropertyCode . "','" . $strSortBy . "','','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
						$process=odbc_exec($sqlconnect, $sqlquery);			
						//echo $sqlquery;
						while(odbc_fetch_row($process)) {
							$ctr++;
...
function save_text()
   {
   var w = frmSAPUploading.cboRealProperty.selectedIndex;
   frmSAPUploading.hidRealPropertyName.value = frmSAPUploading.cboRealProperty.options[w].text;
   cmdBuild_OnClick();
   }
...
function chkSelectAll_OnClick() {
	var ctr
	ctr = frmSAPUploading.hidRowCtr.value;
	for (i=1;i<=ctr;i++) {
		obj = eval("frmSAPUploading.chkSelect" + i);
		if (frmSAPUploading.chkSelectAll.checked == true) {
			obj.checked = true;
		}
		else {
			obj.checked = false;
...
function cmdBuild_OnClick() {
	if (frmSAPUploading.DPC_txtDateFrom.value == "") {
		alert("Please provide Invoice Date From")
		frmSAPUploading.DPC_txtDateFrom.focus()
		return false
	}
	if (frmSAPUploading.DPC_txtDateFrom.value != "") {
		if (isDate(frmSAPUploading.DPC_txtDateFrom.value)==false) {
			frmSAPUploading.DPC_txtDateFrom.focus()
			return false
		}
	}
	
	if (frmSAPUploading.DPC_txtDateTo.value == "") {
		alert("Please provide Invoice Date To")
		frmSAPUploading.DPC_txtDateTo.focus()
		return false
	}
	if (frmSAPUploading.DPC_txtDateTo.value != "") {
		if (isDate(frmSAPUploading.DPC_txtDateTo.value)==false) {
			frmSAPUploading.DPC_txtDateTo.focus()
			return false
		}
	}
		
	if (frmSAPUploading.DPC_txtDateFrom.value != "" && frmSAPUploading.DPC_txtDateTo.value != "") {
		if (CompareDates(frmSAPUploading.DPC_txtDateFrom.value,frmSAPUploading.DPC_txtDateTo.value)==false) {
			frmSAPUploading.DPC_txtDateFrom.focus()
			return false
		}
	}

	frmSAPUploading.hidMode.value = "BUILD";
	frmSAPUploading.submit();
}

function cmdGenerate_OnClick() {
	var j
	j=0
	totalctr = frmSAPUploading.hidRowCtr.value;
	...
	if (j > 0) {
		frmSAPUploading.hidMode.value = "GENERATE";
		frmSAPUploading.submit();
	}
	else {
...
function cmdPost_OnClick() {
	...
	if (j > 0) {
		if (confirm("Are you sure you want to post these invoices? There is no undo for this process.")) {
			frmSAPUploading.hidMode.value = "POST";
			frmSAPUploading.submit();
		}
	}
	else {
		alert("Select invoice/s to post first");
	}
}
...
function cmdSearch_onClick() {
	save_text();
	frmSAPUploading.hidMode.value = "SEARCH";
	frmSAPUploading.submit();
}
