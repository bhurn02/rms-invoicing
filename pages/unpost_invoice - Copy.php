...
if ($strMode != "") {	
	...
	switch ($strMode) {
		case "SEARCH":						
			$sqlquery="exec sp_u_Unpost_Invoice_Search_View 'SEARCH','','" . $dtBillingFrom . "','" . $dtBillingTo . "','" . $strRealPropertyCode . "','" . $strSortBy . "','','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
		...
		case "UNPOST":						
			...
			while ($i <= $_POST["hidRowCtr"]) {
				if (isset($_POST["chkSelect" . strval($i)])) {					
					...
					$sqlquery="exec sp_u_Unpost_Invoice 'UNPOST','" . $strInvoiceNo . "','" . $dtBillingFrom . "','" . $dtBillingTo . "','" . $strRealPropertyCode . "','','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
					//echo $sqlquery;
					$process=odbc_exec($sqlconnect, $sqlquery);					
					$j++;
				}
			...
			if ($j > 0)
				$strMsg = "Invoice/s unposted!";
			else
				$strMsg = "No invoice unposted!";
...
<form name="frmUnpostInvoice" id="frmUnpostInvoice" method="post">
	<div class="mainmenu">	
		<ul>
			  <li class="li_nc"><a name="MODULE NAME">UNPOST INVOICE
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

			 </a></li>	
			  <li class="li_nc"><a href="#" onClick="javascript:cmdSearch_OnClick()">|&nbsp;&nbsp;&nbsp;Search&nbsp;&nbsp;&nbsp;|</a></li>			  			  
			  <li class="li_nc"><a href="#" onClick="javascript:cmdUnpost_OnClick()">Unpost&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|</a></li>			  			  
				...
				<td>
					...
					<table width="900" style="border:1px solid #556b2f;padding: 30px 10px 5px; width:auto;">
						<tr height="30">
							...
							<?php
								$ctrCharge = 0;
								$sqlqueryCharges="select * from m_charges where charge_code in (select charge_code from z_tmp_unpost_invoice_search where isnull(total_amount,0) > 0) order by charge_desc";	
						...
						<?php	
						$ctr = 0;		
						$sqlquery="exec sp_u_Unpost_Invoice_Search 'SEARCH','','" . $dtBillingFrom . "','" . $dtBillingTo . "','" . $strRealPropertyCode . "','" . $strSortBy . "','','" . $uid . "','" . $company_code . "','" . $strIPAddr . "'";	
						$process=odbc_exec($sqlconnect, $sqlquery);			
						//echo $sqlquery;
...
function save_text()
   {
   var w = frmUnpostInvoice.cboRealProperty.selectedIndex;
   frmUnpostInvoice.hidRealPropertyName.value = frmUnpostInvoice.cboRealProperty.options[w].text;
   cmdSearch_OnClick();
   }
...
function chkSelectAll_OnClick() {
	var ctr
	ctr = frmUnpostInvoice.hidRowCtr.value;
	for (i=1;i<=ctr;i++) {
		obj = eval("frmUnpostInvoice.chkSelect" + i);
		if (frmUnpostInvoice.chkSelectAll.checked == true) {
			obj.checked = true;
		}
		else {
			obj.checked = false;
...
function cmdSearch_OnClick() {
	if (frmUnpostInvoice.DPC_txtDateFrom.value == "") {
		alert("Please provide Invoice Date From")
		frmUnpostInvoice.DPC_txtDateFrom.focus()
		return false
	}
	if (frmUnpostInvoice.DPC_txtDateFrom.value != "") {
		if (isDate(frmUnpostInvoice.DPC_txtDateFrom.value)==false) {
			frmUnpostInvoice.DPC_txtDateFrom.focus()
			return false
		}
	}
	
	if (frmUnpostInvoice.DPC_txtDateTo.value == "") {
		alert("Please provide Invoice Date To")
		frmUnpostInvoice.DPC_txtDateTo.focus()
		return false
	}
	if (frmUnpostInvoice.DPC_txtDateTo.value != "") {
		if (isDate(frmUnpostInvoice.DPC_txtDateTo.value)==false) {
			frmUnpostInvoice.DPC_txtDateTo.focus()
			return false
		}
	}
		
	if (frmUnpostInvoice.DPC_txtDateFrom.value != "" && frmUnpostInvoice.DPC_txtDateTo.value != "") {
		if (CompareDates(frmUnpostInvoice.DPC_txtDateFrom.value,frmUnpostInvoice.DPC_txtDateTo.value)==false) {
			frmUnpostInvoice.DPC_txtDateFrom.focus()
			return false
		}
	}

	frmUnpostInvoice.hidMode.value = "SEARCH";
	frmUnpostInvoice.submit();
}

function cmdUnpost_OnClick() {
	var j
	j=0
	totalctr = frmUnpostInvoice.hidRowCtr.value;
	...
	if (j > 0) {
		if (confirm("Are you sure you want to UNPOST these invoices?")) {
			frmUnpostInvoice.hidMode.value = "UNPOST";
			frmUnpostInvoice.submit();
		}
	}
	else {
		alert("Select invoice/s to unpost first");
	}
}
