...
$sqlquery="exec sp_t_Payment_Header 'CLIENT_SEARCH','','','',0,'','','','" . $strKeyword . "','','',0,0,'','',''";	
$process=odbc_exec($sqlconnect, $sqlquery);
				...
				<td>
					<table width="700" style="border:1px solid #556b2f;padding: 30px 10px 5px;">
						...
						while(odbc_fetch_row($process)){
							$tenant_code = replacesinglequote(odbc_result($process,"tenant_code")); 
							$tenant_name = replacesinglequote(odbc_result($process,"tenant_name")); 
...
function tenant(pTenantCode,pTenantName) {
	window.opener.frmPaymentHdr.hidClientCode.value = pTenantCode;
	window.opener.frmPaymentHdr.hidClientName.value = pTenantName;
	window.opener.frmPaymentHdr.txtClientName.value = pTenantName;
	window.close();
}
