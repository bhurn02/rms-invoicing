...
if ($strMode != "") {
	...
	while ($i <= $_POST["hidRowCtr"]) {		
		$sqlquery="exec sp_rpt_Occupancy_Log '" . $_POST["DPC_txtDateFrom"] . "','" . $_POST["DPC_txtDateTo"] . "','" . $_POST["hidCode" . $i] . "','" . $_POST["txtRemarks" . $i] . "','" . $dtCreated . "','" . $uid . "'";	
...
if ($strMode == "GENERATE") {
	//$dtCreated = date("m/d/Y",(strtotime(date("m/d/y", time()))+60*60*24*($OFFSET)));	
	$uid = $sessUserID;
	$sqlquery="exec sp_rpt_Occupancy '" . $_POST["DPC_txtDateFrom"] . "','" . $_POST["DPC_txtDateTo"] . "','" . $dtCreated . "','" . $uid . "'";	
	$process=odbc_exec($sqlconnect, $sqlquery);			
	...
	//die();
	//rpt source file 
	$pdf_file = "t_occupancy" . str_replace("/","",date("m/d/y/H/i/s", time())) . ".xls";
	...
	$creport->ParameterFields(3)->AddCurrentValue ((string)$dtCreated);
	//$creport->ParameterFields(3)->AddCurrentValue ("10/11/2010");
	$creport->ParameterFields(4)->AddCurrentValue ($uid);
	...
	//die();
	//rpt source file 
	$pdf_file = "t_occupancy" . str_replace("/","",date("m/d/y/H/i/s", time())) . ".pdf";
	$my_pdf = $pdf_path . $pdf_file; // RPT export to pdf file 
	$my_pdf_open = $pdf_link . $pdf_file;
	//echo $my_pdf_open;
	//die();
...
$sqlqueryList = "SELECT real_property_code,real_property_name,isnull((select top 1 remarks from rg_occupancy 
	where real_property_code = a.real_property_code order by date_generated desc, tran_date desc),'') as remarks FROM m_real_property a 
	where upper(ltrim(rtrim(isnull(space_type,'A')))) = 'A'  and real_property_code <> 'LNHSE'
...
<form name="frmOccupancy" id="frmOccupancy" method="post">
	<div class="mainmenu">	
		<ul>
			  <li class="li_nc"><a name="MODULE NAME">MANAGEMENT >> OCCUPANCY REPORT
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			 </a></li>	
...
function cmdPrint_OnClick() {
	...
	if (frmOccupancy.DPC_txtDateFrom.value != "") {
		if (isDate(frmOccupancy.DPC_txtDateFrom.value)==false) {
			frmOccupancy.DPC_txtDateFrom.focus()
			return false
		}
	}
	if (frmOccupancy.DPC_txtDateTo.value != "") {
		if (isDate(frmOccupancy.DPC_txtDateTo.value)==false) {
			frmOccupancy.DPC_txtDateTo.focus()
			return false
		}
	}
	
	if (frmOccupancy.DPC_txtDateFrom.value != "" && frmOccupancy.DPC_txtDateTo.value != "") {
		if (CompareDates(frmOccupancy.DPC_txtDateFrom.value,frmOccupancy.DPC_txtDateTo.value)==false) {
			frmOccupancy.DPC_txtDateFrom.focus()
			return false
		}
	}
		
	frmOccupancy.hidMode.value = "PRINT"
	frmOccupancy.submit()
}
...
function cmdGenerate_OnClick() {
	...
	frmOccupancy.hidMode.value = "GENERATE"
	frmOccupancy.submit()
}
